...
class PositionCsvLoggerTests(unittest.TestCase):
    ...
    def test_loads_older_motor_only_log(self):
        names = ("right_knee", "left_knee")
        with tempfile.TemporaryDirectory() as temporary_directory:
            path = Path(temporary_directory) / "motor_positions_old.csv"
            with path.open("w", encoding="utf-8", newline="") as handle:
                writer = csv.writer(handle)
                writer.writerow(
                    [
                        "elapsed_s",
                        "target_right_knee_rad",
                        "target_left_knee_rad",
                        "actual_right_knee_rad",
                        "actual_left_knee_rad",
                    ]
                )
                writer.writerow(["0.02", "0.3", "-0.3", "0.2", "-0.2"])

            elapsed_s, targets, actuals, acceleration, orientation = load_position_log(
                path, names
            )

        np.testing.assert_allclose(elapsed_s, [0.02])
        np.testing.assert_allclose(targets, [[0.3, -0.3]])
        np.testing.assert_allclose(actuals, [[0.2, -0.2]])
        self.assertEqual(acceleration.shape, (1, 0))
        self.assertEqual(orientation.shape, (1, 0))
